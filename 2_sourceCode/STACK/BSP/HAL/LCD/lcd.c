...
lcd_error_t hal_lcd_init(lcd_t *lcd)
{
  ...
  if (gpioError == GPIO_STATE_SUCCESS)
  {
    if (lcd->lcdMode == MODE_8_BIT)
    {
      ...
      if (PORT_STATE_SUCCESS == mcal_port_init(lcd->lcdDataPort, DIR_PORT_OUTPUT, PORT_FULL))
      {
        ...
        else
        {
          ...
          logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"LCD send configure 8 bit mode failed");
        }
      }
      else
      {
        error = LCD_ERROR;
        logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Configure LCD port failed");
      }
    }
    else if (lcd->lcdMode == MODE_4_BIT)
    {
      ...
      if (PORT_STATE_SUCCESS == mcal_port_init(lcd->lcdDataPort, DIR_PORT_OUTPUT, PORT_HIGH_ORDER))
      {
        ...
        {
          /* 4 bit mode is configured */
        }
        else
        {
          error = LCD_command_ERROR;
          logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"send configure 4 bit mode failed");
        }
      }
      else
      {
    ...
    if (error == LCD_SUCCESS)
    {
      ...
      if (LCD_SUCCESS == hal_lcd_sendData(lcd, COMMAND, CURSOR_OFF))
      {
        ...
        {
          error = LCD_command_ERROR;
          logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Failed to send clear command");
        }
      }
      else
      {
        error = LCD_command_ERROR;
        logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Failed to send cursor off");
      }
    }
    else
    {
  ...
  {
    error = LCD_ERROR;
    logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Faild to configure LCD control pins");
  }
...
lcd_error_t hal_lcd_sendData(lcd_t *lcd, lcd_data_types_t type, u8_t data)
{
  ...
  else
  {
    ...
    logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Failed to send command or display");
  }
  ...
  /* disable LCD E=0 */
  if (GPIO_STATE_SUCCESS == mcal_gpio_pin_write(lcd->lcdControlPort, lcd->lcdE, LOW))
  {
    delay_ms(1);
  }
  else
  {
    error = LCD_ERROR;
    logger_write_error_println(LOG_MCAL, TAG, (u8_t *)"Failed to disable E");
  }
